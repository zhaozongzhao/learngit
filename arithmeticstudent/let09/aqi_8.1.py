...
def main():
    ...
    #
    # print(aqi_list)
    header = ['city','AQI','pm25/1h','co/1h','no2/1h','03/8h','s02/1h']
    with open('china_city_aqi2.csv',mode='w',encoding='utf-8',newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for s,i in enumerate(city_list):
            if (s+1)%10 == 0:
                print('已处理{}条，剩余{}'.format(s+1,len(city_list)-(s+1)))
            name = i[0]
            city = i[1]
            aqi_list = get_aqi_list(city)
            row = [name]+aqi_list
